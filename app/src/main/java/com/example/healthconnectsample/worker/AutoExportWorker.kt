...
class AutoExportWorker(
    ...
    private suspend fun performFullExport(healthConnectClient: HealthConnectClient) {
        ...
        val nutritionData = readNutritionRecords(healthConnectManager, startTime, endTime)
        val activeCaloriesData = readActiveCaloriesBurnedRecords(healthConnectManager, startTime, endTime)
    ...
    private suspend fun performDifferentialExport(healthConnectClient: HealthConnectClient) {
        ...
        val nutritionChanges = mutableListOf<Map<String, Any?>>()
        val activeCaloriesChanges = mutableListOf<Map<String, Any?>>()
...
is SleepSessionRecord -> {
                            ...
                            is NutritionRecord -> {
                                nutritionChanges.add(mapOf(
                                    "start_time" to record.startTime.toString(),
                                    "end_time" to record.endTime.toString(),
                                    "name" to (record.name ?: "Unknown"),
                                    "meal_type" to (record.mealType ?: 0),
                                    "calories_kcal" to record.energy?.inKilocalories,
                                ...
                                Log.d(TAG, "  + Nutrition change detected")
                            }
                            is ActiveCaloriesBurnedRecord -> {
                                activeCaloriesChanges.add(mapOf(
                                    "start_time" to record.startTime.toString(),
                                    "end_time" to record.endTime.toString(),
                                    "energy_kcal" to record.energy.inKilocalories,
                                    "source" to record.metadata.dataOrigin.packageName,
                                ...
                                Log.d(TAG, "  + Active Calories change detected")
                            }
                        }
                    }
                    is DeletionChange -> {
            ...
            if (weightChanges.isEmpty() && exerciseChanges.isEmpty() && sleepChanges.isEmpty() &&
                ...
                heartRateChanges.isEmpty() && bloodPressureChanges.isEmpty() && bloodGlucoseChanges.isEmpty() &&
                nutritionChanges.isEmpty() && activeCaloriesChanges.isEmpty() && deletions.isEmpty()) {
...
private suspend fun readRestingHeartRateRecords(
        ...
        return try {
            ...
            records.map { record ->
                mapOf(
                    ...
                    "end_time" to record.endTime.atZone(record.endZoneOffset ?: ZoneId.systemDefault()).toString(),
                    "name" to (record.name ?: "Unknown"),
                    "meal_type" to when(record.mealType) {
                        1 -> "Breakfast"
                        2 -> "Lunch"
                        3 -> "Dinner"
                        4 -> "Snack"
    ...
    private suspend fun readActiveCaloriesBurnedRecords(
        healthConnectManager: HealthConnectManager,
        startTime: Instant,
        endTime: Instant
    ): List<Map<String, Any?>> {
        return try {
            val records = healthConnectManager.readActiveCaloriesBurnedRecords(startTime, endTime)
            records.map { record ->
                mapOf(
                    "start_time" to record.startTime.atZone(record.startZoneOffset ?: ZoneId.systemDefault()).toString(),
                    "end_time" to record.endTime.atZone(record.endZoneOffset ?: ZoneId.systemDefault()).toString(),
            ...
            Log.e(TAG, "Error reading active calories burned records", e)
            emptyList()
        }
    }
    ...
    private fun generateHealthJSON(
        ...
        nutritionData: List<Map<String, Any?>>,
        activeCaloriesData: List<Map<String, Any?>>,
        exportType: String
    ): String {
        return buildString {
            append("{\n")
            ...
            append("  },\n")
            append("  \"nutrition_records\": {\n")
            append("    \"count\": ${nutritionData.size},\n")
            append("    \"data\": ${serializeMapList(nutritionData)}\n")
            append("  },\n")
            append("  \"active_calories_burned_records\": {\n")
            append("    \"count\": ${activeCaloriesData.size},\n")
            append("    \"data\": ${serializeMapList(activeCaloriesData)}\n")
            append("  }\n")
            append("}")
        }
    }
private fun generateDifferentialJSON(
        ...
        nutritionChanges: List<Map<String, Any?>>,
        activeCaloriesChanges: List<Map<String, Any?>>,
        ...
        return buildString {
            ...
            append("  },\n")
            append("  \"nutrition_changes\": {\n")
            append("    \"count\": ${nutritionChanges.size},\n")
            append("    \"data\": ${serializeMapList(nutritionChanges)}\n")
            append("  },\n")
            append("  \"active_calories_burned_changes\": {\n")
            append("    \"count\": ${activeCaloriesChanges.size},\n")
            append("    \"data\": ${serializeMapList(activeCaloriesChanges)}\n")
            append("  },\n")
            append("  \"deletions\": {\n")
