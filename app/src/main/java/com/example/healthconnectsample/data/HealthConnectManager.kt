...
class HealthConnectManager(private val context: Context) {
    ...
    suspend fun readAssociatedSessionData(uid: String): ExerciseSessionData {
        ...
        var fallbackActiveCalories: Energy? = null
        var fallbackAvgHeartRate: Long? = null
        var fallbackMaxHeartRate: Long? = null
        var fallbackMinHeartRate: Long? = null
        ...
        if (needsFallback) {
            ...
            try {
                ...
                if (heartRateRecords.isNotEmpty()) {
                    val allBpmSamples = heartRateRecords.flatMap { it.samples }.map { it.beatsPerMinute }
                    if (allBpmSamples.isNotEmpty()) {
                        fallbackAvgHeartRate = allBpmSamples.average().toLong()
                        fallbackMaxHeartRate = allBpmSamples.maxOrNull()
        ...
        return ExerciseSessionData(
            ...
            totalSteps = aggregateData?.get(StepsRecord.COUNT_TOTAL) ?: fallbackSteps,
            totalDistance = aggregateData?.get(DistanceRecord.DISTANCE_TOTAL) ?: fallbackDistance,
            totalEnergyBurned = aggregateData?.get(TotalCaloriesBurnedRecord.ENERGY_TOTAL) ?: fallbackTotalCalories,
            minHeartRate = aggregateData?.get(HeartRateRecord.BPM_MIN) ?: fallbackMinHeartRate,
            maxHeartRate = aggregateData?.get(HeartRateRecord.BPM_MAX) ?: fallbackMaxHeartRate,
            avgHeartRate = aggregateData?.get(HeartRateRecord.BPM_AVG) ?: fallbackAvgHeartRate,
    ...
    /**
     * Lee registros de VO2Max en un rango de tiempo específico
     */
